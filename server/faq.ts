...
export function searchFAQ(message: string, intent?: IntentType): FAQItem[] {
  ...
  const scored = FAQ_DATABASE.map(faq => {
    ...
    for (const keyword of faq.keywords) {
      if (lowerMessage.includes(keyword)) {
        keywordScore += keyword.length * 10;
      }
    }

    // 2. 如果没有关键词命中，直接跳过（不进入结果）
    if (keywordScore === 0) {
      return { faq, score: 0 };
    }
    ...
    for (const char of lowerMessage) {
      if (questionLower.includes(char)) {
        charScore += 0.5;
      }
    }
    ...
    return { faq, score: keywordScore + charScore + intentBonus };
  });
